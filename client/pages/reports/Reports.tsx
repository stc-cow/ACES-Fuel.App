...
export default function ReportsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (driverPaid !== "All") arr = arr.filter((r) => (driverPaid === "Paid" ? r.settlement === "Paid" : r.settlement !== "Paid"));
    if (!query) return arr;
    const q = query.toLowerCase();
    return arr.filter((r) => [r.siteName, r.driverName, r.phone].some((v) => String(v).toLowerCase().includes(q)));
  ...
  return (
    <AppShell>
      ...
      <div className="px-4 pb-10 pt-4">
        <div className="mb-4 text-sm text-muted-foreground">Report for all done tasks for every single project</div>
          ...
          <CardContent className="p-4">
            ...
            <div className="mt-3 flex items-center gap-2">
              ...
              <div className="ml-auto grid grid-cols-4 gap-3 text-xs text-muted-foreground md:grid-cols-4 lg:grid-cols-4">
                <Input readOnly value={totals.liters} placeholder="Total Liters Used" />
                <Input readOnly value={totals.tasks} placeholder="Total Task Done" />
                <Input readOnly value={totals.paid} placeholder="Total Invoices Paid" />
                <Input readOnly value={totals.notPaid} placeholder="Total Invoices Not Paid" />
              </div>
            </div>

            <div className="mt-4 flex items-center gap-2">
              <Button variant="secondary" onClick={exportCsv}><Download className="mr-2 h-4 w-4" /> Select All</Button>
              <Button variant="outline"><Trash2 className="mr-2 h-4 w-4" /> Deselect all</Button>
              <Button variant="outline">Records Selection</Button>
              <div className="ml-auto flex items-center gap-2">
                <span className="text-sm text-muted-foreground">Search</span>
                <Input value={query} onChange={(e)=>setQuery(e.target.value)} className="h-9 w-56" />
              </div>
            </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-[hsl(var(--primary))] text-white hover:bg-[hsl(var(--primary))]">
                    ...
                    <TableHead className="text-white">Driver Name - Phone</TableHead>
                    <TableHead className="text-white">Start Date</TableHead>
                    <TableHead className="text-white">End Date</TableHead>
                    <TableHead className="text-white">Actual Liter Found in Tank</TableHead>
                    <TableHead className="text-white">Quantity added</TableHead>
                    <TableHead className="text-white">Total invoice</TableHead>
                    <TableHead className="text-white">Determine for settlement</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {filtered.length === 0 && (
                  ...
                  {filtered.map((r) => (
                    <TableRow key={r.id}>
                      <TableCell>
                        <button onClick={()=> setSelected((s)=> ({...s, [r.id]: !s[r.id]}))} aria-label="Select row">
                          {selected[r.id] ? <CheckSquare className="h-4 w-4" /> : <Square className="h-4 w-4" />}
                        </button>
                      </TableCell>
