...
import { Columns2, Download, Plus, Eye, Pencil, Trash2, CheckCircle2, XCircle } from "lucide-react";
...
export default function TechniciansPage() {
  ...
  const [cols, setCols] = useState<Record<ColumnKey, boolean>>({ name: true, phone: true, active: true, settings: true });

  const filtered = useMemo(() => {
    if (!query) return rows;
    const q = query.toLowerCase();
    return rows.filter((r) => [r.name, r.phone].some((v) => String(v).toLowerCase().includes(q)));
  }, [rows, query]);
    ...
    <AppShell>
      ...
      <div className="px-4 pb-10 pt-4">
        <div className="mb-4 flex items-center justify-between">
          <div className="text-sm text-muted-foreground">Manage the technicians and their users</div>
