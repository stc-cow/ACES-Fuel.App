...
import { Eye, Pencil, Trash2, Columns2, Download, Printer, CheckCircle2, XCircle } from "lucide-react";
...
export default function SitesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => [r.name, r.generator, r.driver, r.project, r.city, r.address].some((v) => String(v).toLowerCase().includes(q)));
  }, [rows, query]);
  ...
  const exportCsv = () => {
    const visible = allColumns.filter((c) => cols[c.key] && !["index", "settings", "active"].includes(c.key as string));
  ...
  const openEdit = (r: SiteRow) => {
    setEditing({ id: r.id, site_name: r.name, district: r.city, city: r.address, cow_status: r.cowStatus });
    setEditOpen(true);
  };
  ...
  const saveEdit = async () => {
    ...
    const { error, data } = await supabase
      .from("sites")
      .update({ site_name: editing.site_name, district: editing.district, city: editing.city, cow_status: editing.cow_status })
      .eq("id", editing.id)
      .select("id, site_name, district, city, cow_status")
      .single();
    if (!error && data) {
    ...
    <AppShell>
      ...
      <div className="px-4 pb-10 pt-4">
        <div className="mb-4 flex items-center justify-between">
          <div className="text-sm text-muted-foreground">Manage the site details</div>
            ...
            <div className="overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {current.map((r, idx) => (
                    <TableRow key={r.id}>
                      {cols.index && <TableCell className="font-medium">{(page - 1) * pageSize + idx + 1}</TableCell>}
                      {cols.name && <TableCell className="font-medium">{r.name}</TableCell>}
                      {cols.generator && <TableCell>{r.generator}</TableCell>}
                      {cols.currentLiters && <TableCell>{r.currentLiters}</TableCell>}
                  ...
                  {current.length === 0 && (
                    <TableRow>
                      <TableCell colSpan={allColumns.length} className="text-center text-sm text-muted-foreground">No results</TableCell>
                    </TableRow>
                  )}
                </TableBody>
              </Table>
          ...
          {editing && (
            <div className="space-y-4">
              <div>
                ...
                <Input id="site_name" value={editing.site_name} onChange={(e) => setEditing((s) => (s ? { ...s, site_name: e.target.value } : s))} />
              </div>
              <div>
                <Label htmlFor="district">City (District)</Label>
                <Input id="district" value={editing.district} onChange={(e) => setEditing((s) => (s ? { ...s, district: e.target.value } : s))} />
              </div>
              <div>
                <Label htmlFor="city">Address (City)</Label>
                <Input id="city" value={editing.city} onChange={(e) => setEditing((s) => (s ? { ...s, city: e.target.value } : s))} />
              </div>
              <div>
                <Label>Status</Label>
                <Select value={editing.cow_status} onValueChange={(v) => setEditing((s) => (s ? { ...s, cow_status: v } : s))}>
        ...
        <DialogContent>
          ...
          {viewing && (
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div><span className="text-muted-foreground">Generator:</span> {viewing.generator}</div>
              <div><span className="text-muted-foreground">City (District):</span> {viewing.city}</div>
              <div><span className="text-muted-foreground">Address (City):</span> {viewing.address}</div>
              <div><span className="text-muted-foreground">Status:</span> {viewing.cowStatus}</div>
              <div><span className="text-muted-foreground">Project:</span> {viewing.project}</div>
            </div>
          )}
          <DialogFooter>
